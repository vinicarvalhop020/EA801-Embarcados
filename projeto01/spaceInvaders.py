...
# --- Variáveis do Jogo ---
player_pos = 2          # Posição inicial da nave (coluna 2)
...
def atira(pin):
    global tiro
    if tiro is None:
        tiro = [player_pos, 3]  # Nave está na linha 4, tiro sai da linha 3
...
while True:
    ...
    if tiro:
        for inimigo in inimigos:
            if inimigo[0] == tiro[0] and inimigo[1] == tiro[1]:
                inimigos.remove(inimigo)
                tiro = None
                game_speed = max(0.1, game_speed * 0.9)  # Aumenta dificuldade
                break
    ...
    # Fim de jogo
    if any(inimigo[0] >= 4 for inimigo in inimigos):  # Chegou na nave
        break
